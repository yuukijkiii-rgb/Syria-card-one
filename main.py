...
# --------------------------------------------------
# 1. نظام الحفظ والاسترجاع السحابي عبر MongoDB Atlas
# --------------------------------------------------
MONGO_URI = "mongodb://yuukijkiii_db_user:Vjlmu9moQECP3xw4@cluster0-shard-00-00.uzurgce.mongodb.net:27017,cluster0-shard-00-01.uzurgce.mongodb.net:27017,cluster0-shard-00-02.uzurgce.mongodb.net:27017/?ssl=true&replicaSet=atlas-14knjh-shard-0&authSource=admin&retryWrites=true&w=majority"

mongo_client = MongoClient(MONGO_URI)
db = mongo_client["syria_card_db"]

# مجموعات البيانات (بديلة لملفات الـ JSON)
users_col = db["users"]
categories_col = db["categories"]
products_col = db["products"]
subcategories_col = db["subcategories"]
orders_col = db["orders"]
deposit_methods_col = db["deposit_methods"]
deposit_requests_col = db["deposit_requests"]
providers_col = db["providers"]
settings_col = db["settings"]
banners_col = db["banners"]
category_banners_col = db["category_banners"]
splash_col = db["splash"]

# تعيين الأقسام والإعدادات الافتراضية لأول مرة
if categories_col.count_documents({}) == 0:
    default_categories = ["قسم العاب", "قسم التطبيقات", "قسم رصيد", "قسم رشق", "قسم الاشتراكات", "قسم البريميوم"]
    for c in default_categories:
        categories_col.insert_one({"name": c, "image": ""})
...
if splash_col.count_documents({}) == 0:
    splash_col.insert_one({"_id": "splash", "image": ""})

# =============================================================
# دوال مساعدة للتحويل بين MongoDB و JSON
# =============================================================

def mongo_to_dict(doc):
    """تحويل مستند MongoDB إلى قاموس مع تحويل ObjectId إلى string"""
    if doc is None:
        return None
    doc["_id"] = str(doc["_id"])
    return doc
...
def mongo_list_to_dict(docs):
    """تحويل قائمة مستندات MongoDB إلى قائمة قواميس"""
...
def get_categories():
    return {doc["name"]: doc.get("image", "") for doc in categories_col.find()}

def get_products():
    return list(products_col.find())

def get_subcategories():
    return list(subcategories_col.find())

def get_users():
    return {doc["email"]: {"id": doc.get("id"), "phone": doc.get("phone", ""), "password": doc.get("password"), "balance": doc.get("balance", 0), "api_enabled": doc.get("api_enabled", False)} for doc in users_col.find()}

def get_orders():
    return list(orders_col.find())

def get_deposit_methods():
    return list(deposit_methods_col.find())

def get_deposit_requests():
    return list(deposit_requests_col.find())

def get_providers():
    return list(providers_col.find())

def get_banners():
    return list(banners_col.find())

def get_category_banners():
    return {doc["category"]: doc.get("image", "") for doc in category_banners_col.find()}

def get_splash():
    doc = splash_col.find_one({"_id": "splash"})
    return {"image": doc.get("image", "") if doc else ""}
...
def save_deposit_request(request_data):
    deposit_requests_col.insert_one(request_data)

def save_provider(provider_data):
    providers_col.insert_one(provider_data)

def save_deposit_method(method_data):
    deposit_methods_col.insert_one(method_data)

def save_category(name, image):
    categories_col.update_one({"name": name}, {"$set": {"name": name, "image": image}}, upsert=True)

def save_product(product_data):
    products_col.insert_one(product_data)

def save_subcategory(sub_data):
    subcategories_col.insert_one(sub_data)

def save_banner(image):
    banners_col.insert_one({"image": image})

def save_category_banner(category, image):
    category_banners_col.update_one({"category": category}, {"$set": {"category": category, "image": image}}, upsert=True)

def save_splash(image):
    splash_col.update_one({"_id": "splash"}, {"$set": {"image": image}}, upsert=True)

def save_settings(settings_data):
    settings_col.update_one({"_id": "site_settings"}, {"$set": settings_data}, upsert=True)
...
def delete_banner(index):
    ...
    if 0 <= index < len(banners):
        banners_col.delete_one({"_id": banners[index]["_id"]})

def delete_category_banner(category):
    category_banners_col.delete_one({"category": category})

def delete_deposit_method(index):
    methods = list(deposit_methods_col.find())
    if 0 <= index < len(methods):
        deposit_methods_col.delete_one({"_id": methods[index]["_id"]})
...
# --------------------------------------------------
# 2. نظام الحفظ السحابي عبر MongoDB (بدلاً من JSON)
# --------------------------------------------------
...
# --------------------------------------------------
# 3. واجهة المستخدم العادية (مع إضافة الـ Page Loader)
# --------------------------------------------------
...
<body>
    ...
    <!-- النوافذ المنبثقة - نفس المحتوى السابق -->
...
# --------------------------------------------------
# 5. السيرفر ومعالجة الطلبات
# --------------------------------------------------
class WebAppHandler(BaseHTTPRequestHandler):
    def sync_orders_status_with_api(self):
        changed = False
        if not providers_col.count_documents({}):
            return
        ...
        for order in orders:
            if order.get('status') in ["قيد الانتظار", "تم الارسال للتشيك"] and order.get('order_uuid'):
                ...
                provider = next((p for p in providers if p.get('name') == p_name), providers[0] if providers else None)
                if not provider:
                    continue
                ...
                try:
                    ...
                    if res.get('status') == "OK" and res.get('data') and len(res['data']) > 0:
                        ...
                        if api_status == "accept":
                            orders_col.update_one({"_id": order["_id"]}, {"$set": {"status": "مكتملة"}})
                            changed = True
                        elif api_status == "reject":
                            orders_col.update_one({"_id": order["_id"]}, {"$set": {"status": "مرفوضة"}})
                            u_email = order.get('email')
                            user = users_col.find_one({"email": u_email})
                            if user:
                                cur_b = round(float(user.get('balance', 0)), 3)
                                users_col.update_one({"email": u_email}, {"$set": {"balance": round(cur_b + float(order.get('price', 0)), 3)}})
                            changed = True
                except Exception as e:
    ...
    def do_GET(self):
        if self.path == "/admin/1":
            ...
            pending_deposits = deposit_requests_col.count_documents({"status": "قيد التدقيق"})
            ...
            settings = settings_col.find_one({"_id": "site_settings"})
            if settings:
                del settings["_id"]
            self.send_response(200)
            self.send_header("Content-type", "application/json; charset=utf-8")
            self.end_headers()
            self.wfile.write(json.dumps(settings if settings else {}, ensure_ascii=False).encode("utf-8"))
            ...
            if not auth_token:
                ...
                subcats = list(subcategories_col.find())
                formatted_products = []
            ...
            categories = {doc["name"]: doc.get("image", "") for doc in categories_col.find()}
            self.send_response(200)
            self.send_header("Content-type", "application/json; charset=utf-8")
            self.end_headers()
            ...
            banners = [doc.get("image") for doc in banners_col.find()]
            self.send_response(200)
            self.send_header("Content-type", "application/json; charset=utf-8")
            self.end_headers()
            self.wfile.write(json.dumps(banners, ensure_ascii=False).encode("utf-8"))

        elif self.path == "/api/category_banners":
            cbanners = {doc["category"]: doc.get("image", "") for doc in category_banners_col.find()}
            self.send_response(200)
            self.send_header("Content-type", "application/json; charset=utf-8")
            self.end_headers()
            self.wfile.write(json.dumps(cbanners, ensure_ascii=False).encode("utf-8"))

        elif self.path == "/api/splash":
            splash = splash_col.find_one({"_id": "splash"})
            self.send_response(200)
            self.send_header("Content-type", "application/json; charset=utf-8")
            self.end_headers()
            self.wfile.write(json.dumps({"image": splash.get("image", "") if splash else ""}, ensure_ascii=False).encode("utf-8"))
            ...
            self.send_header("Content-type", "application/json; charset=utf-8")
            self.end_headers()
            self.wfile.write(json.dumps(subcats, ensure_ascii=False).encode("utf-8"))
            ...
            self.send_header("Content-type", "application/json; charset=utf-8")
            self.end_headers()
            self.wfile.write(json.dumps(methods, ensure_ascii=False).encode("utf-8"))

        elif self.path == "/api/get_deposit_requests":
            requests = list(deposit_requests_col.find())
            for r in requests:
                r["_id"] = str(r["_id"])
            self.send_response(200)
            self.send_header("Content-type", "application/json; charset=utf-8")
            self.end_headers()
            self.wfile.write(json.dumps(requests, ensure_ascii=False).encode("utf-8"))
            ...
            self.send_header("Content-type", "application/json; charset=utf-8")
            self.end_headers()
            self.wfile.write(json.dumps(providers, ensure_ascii=False).encode("utf-8"))
            ...
            if user:
                bal = round(float(user.get('balance', 0)), 3)
                u_id = user.get('id', 1001)
                api_en = user.get('api_enabled', False)
                self.send_response(200)
                self.send_header("Content-type", "application/json; charset=utf-8")
                self.end_headers()
            ...
            params = urllib.parse.parse_qs(query)
            email = params.get('email', [''])[0]
            user_deposits = list(deposit_requests_col.find({"email": email}))
    ...
    def do_POST(self):
        ...
        if self.path == "/api/register":
            ...
            new_user_id = 1000 + users_col.count_documents({}) + 1
        ...
        # ============================================================
            ...
            user_balance = round(float(user.get("balance", 0)), 3)
            user_id = user.get("id", 1001)
            ...
            settings_col.update_one({"_id": "site_settings"}, {"$set": {"about_us": about_text}}, upsert=True)
            self.send_response(200)
            self.send_header("Content-type", "application/json; charset=utf-8")
            self.end_headers()
            ...
            settings_col.update_one({"_id": "site_settings"}, {"$set": {
                "telegram_support": data.get('telegram_support', ''),
                "whatsapp_support": data.get('whatsapp_support', ''),
            ...
            for sub in subcategories_col.find({"api_product_id": {"$ne": ""}}):
                base_p = sub.get('base_price')
                if base_p is None:
                    base_p = sub.get('price', 0)
                    subcategories_col.update_one({"_id": sub["_id"]}, {"$set": {"base_price": base_p}})

                new_p = round(float(base_p) * (1 + (margin / 100.0)), 3)
                subcategories_col.update_one({"_id": sub["_id"]}, {"$set": {"price": new_p, "profit_margin": margin}})
                applied_count += 1
            ...
            provider = providers_col.find_one({"name": prov_name}) or providers_col.find_one()
            token = provider.get('token')
            ...
            user = users_col.find_one({"email": email})
            if user:
                current_balance = round(float(user.get('balance', 0)), 3)
                ...
                now = datetime.now()
                order_id = 1000 + orders_col.count_documents({}) + 1
                order_uuid_val = str(uuid.uuid4())
                order_status = "قيد الانتظار"
                system_response = "تم تسجيل الطلب في انتظار القبول"

                if api_product_id and providers_col.count_documents({}):
                    provider = providers_col.find_one({"name": provider_name}) or providers_col.find_one()
                    token = provider.get('token')
                    ...
                    try:
                        ...
                        if api_res.get('status') == "OK":
                            ...
                            if st == 'accept':
                                ...
                                order_status = "مرفوضة"
                                users_col.update_one({"email": email}, {"$set": {"balance": current_balance}})
                ...
                user_pwd = user.get('password', 'غير معروف')
            ...
            if order and order.get('status') in ["قيد الانتظار", "تم الارسال للتشيك"]:
                if action == 'accept':
                    orders_col.update_one({"id": order_id}, {"$set": {"status": "مكتملة"}})
                else:
                    orders_col.update_one({"id": order_id}, {"$set": {"status": "مرفوضة"}})
                    email = order.get('email')
                    price = round(float(order.get('price', 0)), 3)
                    user = users_col.find_one({"email": email})
                    if user:
                        cur_bal = round(float(user.get('balance', 0)), 3)
                        users_col.update_one({"email": email}, {"$set": {"balance": round(cur_bal + price, 3)}})
            ...
            now = datetime.now()
            req = {
                "id": deposit_requests_col.count_documents({}) + 1,
                "email": data.get('email'),
            ...
            deposit_requests_col.insert_one(req)
            self.send_response(200)
            self.send_header("Content-type", "application/json; charset=utf-8")
            self.end_headers()
            self.wfile.write(json.dumps({"status": "success"}).encode("utf-8"))
            ...
            req_id = data.get('id')
            action = data.get('action')
            req = deposit_requests_col.find_one({"id": req_id})

            if req and req.get('status') == "قيد التدقيق":
                if action == 'accept':
                    email = req.get('email')
                    amount = round(float(req.get('amount', 0)), 3)
                    user = users_col.find_one({"email": email})
                    if user:
                        cur_bal = round(float(user.get('balance', 0)), 3)
                        users_col.update_one({"email": email}, {"$set": {"balance": round(cur_bal + amount, 3)}})
                    deposit_requests_col.update_one({"id": req_id}, {"$set": {"status": "مقبول"}})
                else:
                    deposit_requests_col.update_one({"id": req_id}, {"$set": {"status": "مرفوض"}})
            ...
            current_bal = round(float(user.get('balance', 0)), 3)
            if action == 'add':
                new_bal = round(current_bal + amount, 3)
            else:
                new_bal = round(max(0, current_bal - amount), 3)

            users_col.update_one({"email": email}, {"$set": {"balance": new_bal}})
            self.send_response(200)
            self.send_header("Content-type", "application/json; charset=utf-8")
            self.end_headers()
            ...
            if name:
                deposit_methods_col.insert_one({
                    "name": name,
                    "description": description,
            ...
            image = data.get('image')
            if category:
                categories_col.update_one({"name": category}, {"$set": {"name": category, "image": image}}, upsert=True)
            self.send_response(200)
            self.send_header("Content-type", "application/json; charset=utf-8")
            self.end_headers()
            self.wfile.write(json.dumps({"status": "success"}).encode("utf-8"))
            ...
            if cat:
                category_banners_col.delete_one({"category": cat})
            self.send_response(200)
            self.send_header("Content-type", "application/json; charset=utf-8")
            self.end_headers()
            self.wfile.write(json.dumps({"status": "success"}).encode("utf-8"))
            ...
            user = users_col.find_one({"email": ident})
            if not user:
                user = users_col.find_one({"id": int(ident) if ident.isdigit() else -1})

            if user:
                email = user.get("email")
                user_orders = list(orders_col.find({"email": email}))
                user_deposits = list(deposit_requests_col.find({"email": email, "status": "مقبول"}))
                ...
                self.wfile.write(json.dumps({
                    ...
                    "api_enabled": user.get('api_enabled', False),
            ...
            if users_col.find_one({"email": email}):
                users_col.update_one({"email": email}, {"$set": {"api_enabled": enabled}})
                self.send_response(200)
                self.send_header("Content-type", "application/json; charset=utf-8")
                self.end_headers()
                self.wfile.write(json.dumps({"status": "success"}).encode("utf-8"))
