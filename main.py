    ...
    <style>
        ...
        * {
        ...
        .loader-circular-spinner {
            ...
            animation: pageSpin 0.75s cubic-bezier(0.68, -0.55, 0.27, 1.55) infinite;
            z-index: 3;
        }
        ...
        #authModal.active, #registerModal.active {
            display: flex;
        }
        ...
        .register-names-grid .auth-input-custom {
            padding: 15px 18px;
        }

        .auth-green-btn {
            width: 100%;
            background: linear-gradient(90deg, #125e34 0%, #1e824c 50%, #125e34 100%);
...
<head>
    ...
    <style>
        * { font-family: 'Cairo', sans-serif !important; box-sizing: border-box; }
        ...
        .api-pull-select-box label {
            font-size: 0.7rem;
            color: #a1a1aa;
            font-weight: 700;
        }

        .api-pull-select-box select {
            background: #000;
            border: 1px solid #27272a;
            color: #fff;
            padding: 8px;
        ...
        .api-pull-category-card.selected {
            border-color: #4ade80;
            background: rgba(74, 222, 128, 0.05);
        }

        .api-pull-category-header {
            display: flex;
            justify-content: space-between;
            align-items: center;
            cursor: pointer;
        ...
        .api-pull-expand-icon.expanded {
            transform: rotate(90deg);
        }

        .api-pull-children-container {
            display: none;
            flex-direction: column;
            gap: 8px;
            padding-right: 12px;
        ...
        .api-pull-children-container.expanded {
            display: flex;
        }

        .api-pull-floating-action {
            position: fixed;
            bottom: 20px;
            left: 20px;
            right: 20px;
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div class="sub-menu" id="apiMenu">
            ...
            <button class="sub-menu-btn" onclick="openApiPullPage()">سحب API</button>
        </div>
    ...
    <div id="apiPullPage">
        <div class="api-pull-top-bar">
            <span class="api-pull-title">سحب API</span>
            <button class="api-pull-close-btn" onclick="closeApiPullPage()">إغلاق</button>
        </div>

        <div class="api-pull-search-box">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#38bdf8" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
            <input type="text" id="apiPullSearchInput" placeholder="ابحث في الأقسام والمنتجات..." oninput="filterApiPullTree()">
        </div>

        <div class="api-pull-selectors-row">
            <div class="api-pull-select-box">
                <label>المزود API:</label>
                <select id="apiPullProviderSelect" onchange="loadApiPullTree()"></select>
            </div>
            <div class="api-pull-select-box">
                <label>قسم خارجي (اختياري):</label>
                <input type="text" id="apiPullExternalCategoryInput" placeholder="اسم قسم جديد..." style="background:#000; border:1px solid #27272a; color:#fff; padding:8px; border-radius:6px; font-size:0.8rem; outline:none; width:100%;">
            </div>
        </div>

        <div class="api-pull-selectors-row">
            <div class="api-pull-select-box">
                <label>القسم الأب (في المتجر):</label>
                <select id="apiPullParentCategorySelect"></select>
            </div>
            <div class="api-pull-select-box">
                <label>المنتج الأب:</label>
                <select id="apiPullParentProductSelect"></select>
            </div>
        </div>

        <div class="api-pull-hint">
            💡 حدد الأقسام الرئيسية من الشجرة ثم اضغط "سحب المحدد" — سيتم سحب كل ما بداخلها (أقسام فرعية + منتجات + فئات) تلقائياً.
        </div>

        <div class="api-pull-section-title">
            <span>شجرة أقسام API</span>
            <button class="api-pull-refresh-btn" onclick="loadApiPullTree()">🔄 تحديث</button>
        </div>
        <div class="api-pull-tree-container" id="apiPullTreeContainer">
            <div class="api-pull-empty">اختر مزود API لبدء السحب</div>
        </div>

        <button class="api-pull-floating-action" id="apiPullFloatingBtn" onclick="executeApiPull()">
            سحب المحدد (<span id="apiPullSelectedCount">0</span>)
        </button>
    </div>
    ...
    <script>
        ...
        function openApiPullPage() {
            toggleSidebar();
            document.getElementById('apiPullPage').style.display = 'flex';
            document.getElementById('apiPullSearchInput').value = '';
            document.getElementById('apiPullExternalCategoryInput').value = '';
            window._apiPullState.selected = new Set();
            window._apiPullState.expanded = new Set();
            window._apiPullState.tree = [];
            window._apiPullState.rawProducts = [];
            updateApiPullFloatingBtn();

            fetch('/api/get_providers')
                .then(r => r.json())
                .then(providers => {
                    const provSelect = document.getElementById('apiPullProviderSelect');
                    provSelect.innerHTML = '<option value="">اختر مزود API...</option>';
                    providers.forEach(p => {
                        provSelect.innerHTML += `<option value="${p.name}">${p.name}</option>`;
                    });
                    ...
                    fetch('/api/categories')
                        .then(r => r.json())
                        .then(cats => {
                            const catSel = document.getElementById('apiPullParentCategorySelect');
                            catSel.innerHTML = '';
                            Object.keys(cats).forEach(k => {
                                catSel.innerHTML += `<option value="${k}">${k}</option>`;
                            });
                        });
                    ...
                    fetch('/api/products')
                        .then(r => r.json())
                        .then(prods => {
                            const prodSel = document.getElementById('apiPullParentProductSelect');
                            prodSel.innerHTML = '';
                            prods.forEach(p => {
            ...
            document.getElementById('apiPullTreeContainer').innerHTML = '<div class="api-pull-empty">اختر مزود API لبدء السحب</div>';
        }

        function closeApiPullPage() {
            document.getElementById('apiPullPage').style.display = 'none';
        }

        async function loadApiPullTree() {
            const providerName = document.getElementById('apiPullProviderSelect').value;
            const container = document.getElementById('apiPullTreeContainer');

            if (!providerName) {
                container.innerHTML = '<div class="api-pull-empty">اختر مزود API لبدء السحب</div>';
                window._apiPullState.rawProducts = [];
                window._apiPullState.providerName = '';
                window._apiPullState.selected = new Set();
                window._apiPullState.tree = [];
                updateApiPullFloatingBtn();
                return;
            ...
            container.innerHTML = '<div class="api-pull-loader">جاري جلب المنتجات من المزود...</div>';
            ...
            try {
                ...
                if (!provider) {
                    container.innerHTML = '<div class="api-pull-empty">المزود غير موجود</div>';
                    return;
                }

                window._apiPullState.providerName = providerName;
                window._apiPullState.providerToken = provider.token;
                window._apiPullState.providerUrl = provider.url;
                ...
                if (pData.status === 'success' && Array.isArray(pData.products)) {
                    window._apiPullState.rawProducts = pData.products;
                    console.log('عدد المنتجات المجلوبة:', pData.products.length);
                    buildApiPullTree();
                    renderApiPullTree();
                } else {
                    container.innerHTML = '<div class="api-pull-empty">فشل جلب المنتجات من المزود</div>';
                }
            } catch (e) {
                console.error('خطأ في تحميل الشجرة:', e);
                container.innerHTML = '<div class="api-pull-empty">تعذر الاتصال بالمزود</div>';
            }
        }

        function buildApiPullTree() {
            const products = window._apiPullState.rawProducts || [];
            ...
            if (sampleProduct.parent_id === undefined) {
                if (sampleProduct.parentId !== undefined) parentField = 'parentId';
                else if (sampleProduct['الاب'] !== undefined) parentField = 'الاب';
                else if (sampleProduct.category_id !== undefined) parentField = 'category_id';
                else parentField = null;
            }

            function mapProductToNode(p) {
                return {
                    id: p.id,
                    name: p.name || p["الاسم"] || "-",
                    price: p.price !== undefined ? p.price : (p["السعر"] || 0),
                    base_price: p.base_price !== undefined ? p.base_price : (p["السعر_الأساسي"] || p.price || 0),
                    params: p.params || [],
                    category_name: p.category_name || p["اسم_الفئة"] || '',
                    product_type: p.product_type || p["نوع_المنتج"] || 'package',
                    available: p.available === true || p["متاح"] === true,
                    category_img: p.category_img || '',
                    parent_id: parentField ? p[parentField] : 0,
                    raw: p,
                    children: []
                };
            }
            ...
            if (!parentField) {
                window._apiPullState.tree = products.map(mapProductToNode);
                return;
            }

            const roots = products.filter(p => {
                const pid = p[parentField];
                return pid === 0 || pid === "0" || pid === null || pid === undefined || pid === "";
            });
            ...
            function buildChildren(parentId) {
                return products.filter(p => {
                    const pid = p[parentField];
                    if (pid === undefined || pid === null || pid === "" || pid === 0 || pid === "0") return false;
                    return String(pid) === String(parentId);
                });
            }

            function buildNode(prod) {
                const node = mapProductToNode(prod);
                const children = buildChildren(prod.id);
            ...
            const allNodes = roots.map(buildNode);

            const topLevelIds = new Set();
            allNodes.forEach(n => {
                topLevelIds.add(String(n.id));
                n.children.forEach(c => topLevelIds.add(String(c.id)));
            });

            const orphans = products.filter(p => {
                const pid = p[parentField];
                return !rootIds.has(String(p.id)) && !topLevelIds.has(String(p.id)) && pid !== undefined && pid !== null && pid !== "";
            });

            orphans.forEach(o => {
                allNodes.push(mapProductToNode(o));
            });

            window._apiPullState.tree = allNodes;
        }

        function renderApiPullTree() {
            const container = document.getElementById('apiPullTreeContainer');
            const tree = window._apiPullState.tree || [];

            if (tree.length === 0) {
                container.innerHTML = '<div class="api-pull-empty">لا توجد أقسام رئيسية لدى هذا المزود</div>';
                return;
            }

            container.innerHTML = '';
            tree.forEach(node => {
                container.appendChild(renderApiPullNode(node));
            });
        }

        function renderApiPullNode(node) {
            const card = document.createElement('div');
            card.className = 'api-pull-category-card';
            const isSelected = window._apiPullState.selected.has('cat_' + node.id);
            if (isSelected) card.classList.add('selected');

            const hasChildren = node.children && node.children.length > 0;
            const isExpanded = window._apiPullState.expanded.has('cat_' + node.id);
            const childCount = node.children ? node.children.length : 0;

            const header = document.createElement('div');
            header.className = 'api-pull-category-header';
            header.innerHTML = `
                <div class="api-pull-category-name">
                    ${hasChildren ? `<span class="api-pull-expand-icon ${isExpanded ? 'expanded' : ''}">◀</span>` : '<span style="width:16px; display:inline-block;"></span>'}
                    <span class="api-pull-node-name">${node.name}</span>
                    ${hasChildren ? `<span class="api-pull-category-count">${childCount} عنصر</span>` : ''}
                </div>
                <div class="api-pull-card-actions">
                    ${isSelected 
                        ? `<button class="api-pull-deselect-btn" data-action="deselect" data-key="cat_${node.id}">إلغاء التحديد</button>`
                        : `<button class="api-pull-select-btn" data-action="select" data-key="cat_${node.id}">تحديد</button>`
                    }
                </div>
            `;

            header.querySelector('.api-pull-category-name').addEventListener('click', (e) => {
                if (hasChildren) {
                    e.stopPropagation();
                    toggleApiPullExpand(node.id);
                }
            });
            ...
            if (btn) {
                btn.addEventListener('click', (e) => {
                    ...
                    if (action === 'select') selectApiPullItem(key);
                    else deselectApiPullItem(key);
                });
            }
            ...
            if (hasChildren) {
                ...
                childrenContainer.className = 'api-pull-children-container' + (isExpanded ? ' expanded' : '');
                node.children.forEach(child => {
                    childrenContainer.appendChild(renderApiPullNode(child));
                });
        ...
        function toggleApiPullExpand(nodeId) {
            const key = 'cat_' + nodeId;
            if (window._apiPullState.expanded.has(key)) {
                window._apiPullState.expanded.delete(key);
            } else {
                window._apiPullState.expanded.add(key);
            }
            renderApiPullTree();
            filterApiPullTree();
        }

        function selectApiPullItem(key) {
            window._apiPullState.selected.add(key);
            renderApiPullTree();
            filterApiPullTree();
            updateApiPullFloatingBtn();
        }

        function deselectApiPullItem(key) {
            window._apiPullState.selected.delete(key);
            renderApiPullTree();
            filterApiPullTree();
            updateApiPullFloatingBtn();
        }

        function updateApiPullFloatingBtn() {
            const count = window._apiPullState.selected.size;
            document.getElementById('apiPullSelectedCount').innerText = count;
            const btn = document.getElementById('apiPullFloatingBtn');
            if (count > 0) {
        ...
        function filterApiPullTree() {
            const query = document.getElementById('apiPullSearchInput').value.trim().toLowerCase();
            if (!query) {
                renderApiPullTree();
                return;
            }
            ...
            function nodeMatches(node) {
                ...
                if (node.children && node.children.some(nodeMatches)) return true;
                return false;
            }

            const filtered = tree.filter(nodeMatches);
            if (filtered.length === 0) {
                container.innerHTML = '<div class="api-pull-empty">لا توجد نتائج مطابقة للبحث</div>';
                return;
            }
        ...
        async function executeApiPull() {
            const selected = Array.from(window._apiPullState.selected);
            if (selected.length === 0) {
                alert('يرجى تحديد عنصر واحد على الأقل للسحب');
                return;
            }

            const providerName = window._apiPullState.providerName;
            const parentCategory = document.getElementById('apiPullParentCategorySelect').value;
            const parentProduct = document.getElementById('apiPullParentProductSelect').value;
            const externalCategory = document.getElementById('apiPullExternalCategoryInput').value.trim();

            if (!providerName) {
                alert('يرجى اختيار مزود API');
                return;
            }

            if (!parentCategory && !externalCategory) {
                alert('يرجى تحديد قسم أب أو كتابة قسم خارجي');
                return;
            }
            ...
            try {
                const res = await fetch('/api/pull_from_api', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                ...
                const data = await res.json();
                if (data.status === 'success') {
                    alert('تم سحب ' + data.pulled_count + ' عنصر بنجاح!');
                    window._apiPullState.selected = new Set();
                    updateApiPullFloatingBtn();
                    loadApiPullTree();
                    loadCategories();
                    ...
                    alert('فشل السحب: ' + (data.message || 'خطأ غير معروف'));
                }
            } catch (e) {
...
class WebAppHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/api/register":
            ...
            self.send_header("Content-type", "application/json; charset=utf-8")
            self.end_headers()
            self.wfile.write(json.dumps({"status": "success", "balance": 0, "user_id": new_user_id}).encode("utf-8"))
        ...
        elif self.path == "/api/pull_from_api":
            ...
            external_category = data.get('external_category', '').strip()

            if not provider_name or not selected_keys:
                self.send_response(200)
                self.send_header("Content-type", "application/json; charset=utf-8")
                self.end_headers()
                self.wfile.write(json.dumps({"status": "error", "message": "بيانات ناقصة"}).encode("utf-8"))
                return
            ...
            if not provider:
                ...
                self.send_header("Content-type", "application/json; charset=utf-8")
                self.end_headers()
                self.wfile.write(json.dumps({"status": "error", "message": "المزود غير موجود"}).encode("utf-8"))
                return
            ...
            try:
                products = make_api_request(base_url + "client/api/products", token)
            except Exception as e:
                self.send_response(200)
                self.send_header("Content-type", "application/json; charset=utf-8")
                self.end_headers()
                self.wfile.write(json.dumps({"status": "error", "message": f"فشل جلب المنتجات: {str(e)}"}).encode("utf-8"))
                return
            ...
            def get_children(pid):
                result = []
                for p in products:
                    pid_val = p.get('parent_id')
                    if pid_val is None or pid_val == "" or pid_val == 0 or pid_val == "0":
                        continue
                    if str(pid_val) == str(pid):
                        result.append(p)
                return result

            target_root_category = external_category if external_category else parent_category
            if not target_root_category:
                self.send_response(200)
                self.send_header("Content-type", "application/json; charset=utf-8")
                self.end_headers()
                self.wfile.write(json.dumps({"status": "error", "message": "يرجى تحديد قسم أب أو كتابة قسم خارجي"}).encode("utf-8"))
                return

            if external_category and external_category not in CATEGORIES_DATA:
                CATEGORIES_DATA[external_category] = ""
                save_json_file(CATEGORIES_FILE, CATEGORIES_DATA)

            pulled_count = 0

            def get_or_create_product(product_name, category_name, image=''):
                for p in PRODUCTS_DATA:
                    if p.get('name') == product_name and p.get('category') == category_name:
                        return p
                new_p = {"name": product_name, "category": category_name, "image": image}
                PRODUCTS_DATA.append(new_p)
                return new_p

            def apply_profit(api_price):
                profit = 0
                for sub in SUBCATEGORIES_DATA:
                    if sub.get('profit_margin') is not None:
                        profit = sub.get('profit_margin')
                        break
                return round(float(api_price) * (1 + (profit / 100.0)), 3)

            def process_node(node_id, root_cat, parent_prod_name):
                nonlocal pulled_count
                prod = find_product(node_id)
                if not prod:
                    return
                p_name = prod.get('name') or prod.get('الاسم') or 'غير معروف'
                p_price = prod.get('price', 0)
                p_img = prod.get('category_img', '')
                p_params = prod.get('params') or []
                desc = " - ".join([str(x) for x in p_params]) if p_params else ""

                children = get_children(node_id)

                if children:
                    get_or_create_product(p_name, root_cat, p_img)
                    for ch in children:
                        process_node(ch.get('id'), root_cat, p_name)
                else:
                    target_prod_name = parent_prod_name if parent_prod_name else p_name
                    if not parent_prod_name:
                        get_or_create_product(p_name, root_cat, p_img)

                    exists = False
                    for sub in SUBCATEGORIES_DATA:
                        if sub.get('product') == target_prod_name and sub.get('api_product_id') == str(node_id):
                            exists = True
                            new_price = apply_profit(p_price)
                            if float(new_price) > float(sub.get('price', 0)):
                                sub['price'] = new_price
                                sub['base_price'] = p_price
                            if desc:
                    ...
                    if not exists:
                        SUBCATEGORIES_DATA.append({
                            "name": p_name,
                            "price": apply_profit(p_price),
                            "base_price": p_price,
                            "product": target_prod_name,
                            "provider_name": provider_name,
                            "api_product_id": str(node_id),
                            "description": desc,
                            "image": p_img
                        })
                        pulled_count += 1
            ...
            for key in selected_keys:
                if key.startswith('cat_'):
                    node_id = key.replace('cat_', '')
                    process_node(node_id, target_root_category, '')
            ...
            self.send_header("Content-type", "application/json; charset=utf-8")
            self.end_headers()
            self.wfile.write(json.dumps({"status": "success", "pulled_count": pulled_count}, ensure_ascii=False).encode("utf-8"))
